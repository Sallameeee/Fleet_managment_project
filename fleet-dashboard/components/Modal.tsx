...
export default function Modal({ open, onClose, title, children, footer, size = "md" }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
      ...
      <div className={`relative z-10 flex max-h-[calc(100dvh-1rem)] w-full ${maxW} flex-col rounded-2xl border border-ink-800 bg-ink-900 shadow-2xl sm:max-h-[calc(100dvh-2rem)]`}>
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-ink-800 px-4 py-3 sm:px-6 sm:py-4">
        ...
        {footer && <div className="shrink-0 border-t border-ink-800 px-4 py-3 sm:px-6">{footer}</div>}
      </div>
    </div>
  );
}
